...
const Carousel = (props) => {
  ...
  if (show > 2) {
    ...
    if (windowSize.innerWidth <= 1000 && windowSize.innerWidth > 0) {
      show = 1;
      carouselCountDisplay = `show-${show}`;
    } else {
      carouselCountDisplay = `show-${show}`;
      ...
      <div className="carousel-button-container space-x-3">
        <button
          ...
          className={`left-arrow ${
            currentIndex > 0 && "border-black cursor-pointer"
          }`}
        >
          <FaChevronLeft
        ...
        <button
          ...
          className={`right-arrow ${
            currentIndex < Length - show && "border-black cursor-pointer"
          }`}
        >
          <FaChevronRight
            className={`text-[#bababa] ${
              currentIndex < Length - show && "text-black"
            }`}
          />
        </button>
      </div>
